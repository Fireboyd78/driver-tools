...
namespace IMGRipper
{
    sealed class IMGFile
    {
        ...
        public class XBoxDataEntry : DataEntry
        {
            public int ArchiveIndex { get; set; }

            public string ArchiveExtension
            {
                get { return String.Format("L{0:D2}", ArchiveIndex); }
            }
        }
        ...
        public bool IsXBoxVersion { get; set; }
            ...
            using (var fs = File.Open(FileName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                foreach (var entry in Entries)
                {
                    if (!Program.VerboseLog)
                    {
                        ...
                        Console.Write("{0} / {1}", idx, Entries.Count);
                    }
        ...
        public void SaveTableASCII()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (IsXBoxVersion)
                    sb.AppendFormat("/{0:D2}", (entry as XBoxDataEntry).ArchiveIndex);
        ...
        private unsafe void Decrypt()
        {
            using (var fs = File.Open(FileName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                ...
                IsXBoxVersion = (FileInfo.Length < 28672);
                ...
                fixed (byte* p = Buffer)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        DataEntry entry = (IsXBoxVersion) ? new XBoxDataEntry() : new DataEntry();
                        ...
                        if (IsXBoxVersion)
                        {
                            ...
                            (entry as XBoxDataEntry).ArchiveIndex = data & 0xFF;
                            entry.Offset = (data >> 4) & 0xFFFFFF;
