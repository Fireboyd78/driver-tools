...
namespace Zartex
{
    ...
    public class MissionSummaryData : SpoolableResource<SpoolableBuffer>
    {
        ...
        {
            int bufSize = Spooler.Size;
            
            if (HasDensityData)
                bufSize += (sizeof(int) * 6);

            using (var buffer = new MemoryStream(bufSize))
            {
                buffer.WriteFloat(StartPosition[0]);
                buffer.WriteFloat(StartPosition[1]);

                buffer.Write((int)CityType);

                buffer.Write(MissionId);
                buffer.Write(MissionLocaleId);

                if (HasDensityData)
                {
                    buffer.Write(DensityDataMagic);

                    // combine value and padding into one
                    buffer.Write((0xCCCCCC << 8) | ((DensityOverride) ? 1 : 0));

                    buffer.Write(ParkedCarDensity);
                    buffer.Write(AttractorParkedCarDensity);

                    buffer.WriteFloat(PingInRadius);
                    buffer.WriteFloat(PingOutRadius);
                }

                Spooler.SetBuffer(buffer.ToArray());
            }
        }
    }
}
